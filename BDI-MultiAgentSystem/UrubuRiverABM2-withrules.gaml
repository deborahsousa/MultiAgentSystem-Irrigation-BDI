...
//Files: matrices and shapefiles
	...
	date starting_date <- "2022-05-01";
	date current_date <- starting_date update: update_date();
	float step <- 1 #day;
	int nb_days <- 123;
	...
	float f3_daily_withdrawal update: sum(Farmer[2].owned_pumps collect each.daily_withdrawal);//TODO
	float f5_daily_withdrawal update: sum(Farmer[4].owned_pumps collect each.daily_withdrawal);//TODO
	...
	//scenarios
	string scenario <- "S0" ;//among: ["S0","S1","S2","S3","S4","S5"]; //simulation scenarios
	...
	/*action load_parameters { //changing parameters in simulation scenarios
		...
		reflex save_daily_data {		
			...
			save[all_pumps_daily_withdrawal] to: "../results/daily_withdrawal_pumps_all.csv"  type:csv rewrite:false;
			save[withdrawal_pumps_NC] to: "../results/daily_withdrawal_pumpsNC.csv"  type:csv rewrite:false;
			save[withdrawal_pumps_CP] to: "../results/daily_withdrawal_pumpsCP.csv" type:csv rewrite:false;
			save[withdrawal_pumps_CI] to: "../results/daily_withdrawal_pumpsCI.csv" type:csv rewrite:false;
			save[f1_daily_withdrawal] to: "../results/daily_withdrawal_farmer1.csv" type:csv rewrite:false;
			save[f3_daily_withdrawal] to: "../results/daily_withdrawal_farmer3.csv" type:csv rewrite:false;
			save[f5_daily_withdrawal] to: "../results/daily_withdrawal_farmer5.csv" type:csv rewrite:false;*/
		...
		reflex end_simulation when:cycle = 100*nb_days { // pause criteria to GUI experiments
			do pause;
		}
}
...
species Pump {
	...
	reflex update_withdrawal when: (twoweeks_count < 9) { //TODO when belief "yellow rule" or "red rule" is true, different criteria for each profile for it to be true
		loop j from:0 to:7 {
