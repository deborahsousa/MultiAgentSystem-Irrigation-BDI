...
global {
	...
	int col_level;
	float current_level <- 500 update:update_my_level();//level variation at the reference gauging station [cm] //might be given as an input of a hydrological model output
	...
	date my_day update:current_date;
		
	//counters and time steps (dry season)
	date starting_date <- date("2020-04-29");
	float step <- 1 #day;
	...
	action update_my_level type: float {
		if cycle>0 {
			float my_level <- level_series[col_level,cycle]; 
			return my_level;
		} else {
			return 500;
		} 
...
experiment repetitions type: batch repeat: 2 autorun: false keep_seed:true until: cycle = nb_days+2 { 	
	reflex end_of_runs {
		int sim <- 0;
